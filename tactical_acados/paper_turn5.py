...
# -*- coding: utf-8 -*-
"""
Paper scenario: Turn 5 overtaking — LOCAL SEGMENT simulation.
Adapted from paper_turn678.py — only s ranges and default params differ.

Usage:
  python paper_turn5.py [--steps 800] [--run-id run1]
"""
...
# ======================================================================
def run_sim(scenario_name, max_steps=800):
...
def setup_ieee():
    ...
    rcParams.update({
        ...
        'lines.linewidth': 0.7, 'figure.dpi': 300, 'savefig.dpi': 300,
        'savefig.bbox': 'tight', 'axes.grid': True, 'grid.alpha': 0.25,
        'grid.linewidth': 0.3, 'axes.linewidth': 0.5,
...
def plot_trajectory(rec, events, track_handler, save_path,
                    s_plot_min=1090, s_plot_max=1560, title=""):
    setup_ieee()
    ...
    t_arr = np.array(rec['time']); N = len(ego_x)
    ...
    def draw_track(ax, lw=0.6):
        ...
        ax.plot(xyz_c[:, 0], xyz_c[:, 1], '--', color='#cccccc', linewidth=lw*0.4)
    ...
    ax_main.legend(loc='upper left', fontsize=5.5, framealpha=0.8, handlelength=1.5)
    ...
    ax_main.set_xlim(all_x.min()-12, all_x.max()+12)
    ax_main.set_ylim(all_y.min()-12, all_y.max()+12)
    ...
    for k, (ax_z, box, lbl, rc) in enumerate(
        ...
        for spine in ax_z.spines.values():
            spine.set_edgecolor(rc); spine.set_linewidth(1.0)
...
def plot_timeseries(rec, events, save_path, title=""):
    ...
    ot_vlines(ax)
    ax.set_ylabel('$\\Delta s$ [m]'); ax.set_xlabel('Time [s]')
    ...
    ax.plot(t, cl, '-', color='#059669', linewidth=0.4, alpha=0.6, label='Corridor')
    ...
    ot_vlines(ax)
    ax.set_ylabel('Lateral $n$ [m]'); ax.set_xlabel('Time [s]')
...
def make_gif(rec, track_handler, save_path,
    ...
    for fi, idx in enumerate(frame_indices):
        ...
        ax.plot(ego_x[idx], ego_y[idx], 'o', color='#1f77b4',
                markersize=7, zorder=5, markeredgecolor='w', markeredgewidth=0.5)
        ...
        for oid in opp_ids:
            ...
            ax.plot(ox[idx], oy[idx], 's', color=c,
                    markersize=6, zorder=5, markeredgecolor='w', markeredgewidth=0.4)
            ax.annotate(f'Opp{oid}', (ox[idx], oy[idx]),
                       fontsize=5, ha='center', va='bottom',
                       xytext=(0, 3), textcoords='offset points', color=c)
...
def main():
    parser = argparse.ArgumentParser(
        description='Turn 5 local segment overtaking demo')
    parser.add_argument('--steps', type=int, default=800)
    parser.add_argument('--run-id', type=str, default='run1')
    parser.add_argument('--scenario', type=str, default='scenario_turn5')
    parser.add_argument('--no-gif', action='store_true')
    args = parser.parse_args()

    outdir = os.path.join(dir_path, 'figures', 'turn5')
    ...
    plot_trajectory(rec, events, th,
                    ...
                    s_plot_min=1090, s_plot_max=1560,
                    title='Turn 5 Overtaking')
